...
class BaseViewSet(viewsets.GenericViewSet):
    ...
    ordering: str
    permission_classes_map: typing.Dict[
        str, typing.List[typing.Type[permissions.BasePermission]]]
    ...
    def get_permissions(self):
        if hasattr(self, "permission_classes_map"):
            return [permission() for permission in
                    self.permission_classes_map[self.action]]
        return super().get_permissions()

    def _update(self, instance,
                success_status=status.HTTP_200_OK) -> response.Response:
        self.serializer = self.get_serializer(instance=instance,
    ...
    def _create(self,
                success_status=status.HTTP_201_CREATED) -> response.Response:
    ...
    def _destroy(self, obj=None) -> response.Response:
        instance = obj or self.get_object()
        instance.delete()
        return response.Response(status=status.HTTP_204_NO_CONTENT)


    def _grouped_list(self, group_by: str):
        queryset = self.filter_queryset(self.get_queryset())
        ...
        if page is not None:
            self.serializer = self.get_serializer(page, many=True)
            for obj in self.serializer.data:
                temp_obj = obj
                for attr in group_by.split("."):
                    temp_obj = temp_obj[attr]
                res[temp_obj].append(obj)
